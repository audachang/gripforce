#!/bin/tcsh -xef
if ( $#argv != 6 ) then
	echo "usage: ../ana_scripts/roi_ana_model_Tstat.sh <condstr> <atlas> <tpval01> <subjfn> <NNval> <sesid>"
	echo "Example: ../ana_scripts/roi_ana_model_Tstat.sh R40 FS.MNI2009c_asym 2.1633 list_afni_ses-01.txt 1 ses-01"
	echo condstr: the condition parameter to be extracted
	echo atlas: name of the whole brain bask to use
    ...
    echo 
	echo updated on 2023-06-25 by EC
...
set anadir1 = $gpdir/test.CSPLINzero.3dMEMA_N=${Num}.R-L.$ses.results
set anadir2 = $gpdir/test.CSPLINzero.3dMEMA_N=${Num}.$atlas.$ses.results
...
set atlasdir = "/media/DATA3/GripForce/atlas_templates"
if (! -d $anadir2) then
    echo $anadir2 not found. Creating one ...
    mkdir $anadir2
endif
cp $atlasdir/FS* $anadir2/
cp $anadir1/*.BRIK $anadir2/
cp $anadir1/*.HEAD $anadir2/



3dcalc -a $anadir2/"${condstr}_sel#0_Tstat+tlrc" 			\
	-b $anadir2/${atlas}_mask+tlrc  			\
		-prefix $anadir2/"${condstr}_sel#0_atlas" 	\
...
3dROIstats -nzvoxels  -nomeanout -longnames \
	-mask $anadir2/${atlas}_mask+tlrc \
	$anadir2/"${condstr}_sel#0_Tstat+tlrc" > \
	$anadir2/"${condstr}_sel#0_Tstat=${tpval01}_atlas_ROI_count.txt"
echo 3dROIstats end
