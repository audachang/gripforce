...
#    -frac 1.0
#ls -l  ${path_main}/group_results/mask_intersection_up2date+tlrc.HEAD 
3dcopy -overwrite \
  ${path_main}/group_results/mask_intersection_func_up2date+tlrc.HEAD \
  ./mask.nii.gz
...
grep -h "$gpattern" ${path_main}/subj.sub*/ses-01/sub*.${model}.results/blur_est*   \
    | cut -d\  -f1-3                              \
    > group_ACF_ests.1D
